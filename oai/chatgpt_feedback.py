...
def parse_gpt(res):
    ...
    for sp in res_split:
        sp_split = sp.split("\n")
        domain, summary, task_type, reasoning = sp_split[0], sp_split[1], sp_split[2], sp_split[3]

        assert domain.startswith('[domain]')
        assert summary.startswith('[summary]')
        assert task_type.startswith('[task type]')
        assert reasoning.startswith('[reasoning requirements]')

        domain = domain[8:]
        summary = summary[9:]
        task_type = task_type[11:]
        reasoning = reasoning[24:]
    ...
    return domains, summaries, task_types, reasonings
...
def decoder_gpt(input, temp, engine, max_length):
    ...
    for i in range(MAX_API_RETRY):
        try:
            response = client.chat.completions.create(model=engine, # gpt4-32k
            messages=[{
                "role":"user",
                "content":input,
            }],
...
def demo_selection(demo_file, num_demo):
    ...
    with open(demo_file, 'r', encoding='utf-8') as f:
        for j in f.readlines():
            ...
            output.append('[domain]' + j['domain'] + '\n[summary]' + j['summary'] + '\n[task type]' + j['label'] + '\n[reasoning requirements]' + j['reasoning'])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_file", default="sample.json", help="data to be analyzed, in this project, shareGPT"
        )
    parser.add_argument(
    ...
    parser.add_argument(
        "--engine", default="gpt-3.5-turbo-1106", help="GPT engine to be used"
    )
    ...
    for item_idx in trange(0, len(raw_data), args.step):
        ...
        try:
            domain, summary, task_type, reasoning = parse_gpt(res)
        except Exception as e:
            logger.error(e)
            continue

        try:
            assert len(domain) == len(summary) == len(task_type) == len(step_res) == len(reasoning)
        except:
            continue
        ...
        for res_id in range(len(step_res)):
            ...
            step_res[res_id]['reasoning'] = reasoning[res_id]
